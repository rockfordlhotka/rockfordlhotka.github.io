...
**Consider Aspire.** If you use [Aspire](https://aspire.dev) to orchestrate your app, much of this comes for free. The service defaults project wires up OpenTelemetry logging, tracing, and metrics for every service, and the Aspire dashboard collects all of that telemetry in one place. The agent can get at the same data through the Aspire CLI or the [Aspire MCP server](https://aspire.dev/get-started/aspire-mcp-server/), which let it list resources, read structured logs, and inspect distributed traces without anyone copying and pasting terminal output. And when it's time for the human outer loop, that same dashboard gives you a nice way to review what actually happened: which services were called, how long each step took, and what got logged along the way.
